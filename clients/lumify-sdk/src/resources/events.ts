...
export interface GetEventParams {
   ...
   * Costs +1 credit for a single book, +2 for `bookmaker: "all"` or a
...
export interface BatchGetEventsParams {
   ...
   * Costs +1 credit per event for a single book, +2 for `bookmaker: "all"` or a
   * comma-separated list, when odds are available.
   */
  includeOdds?: boolean;
...
export class EventsResource {
   ...
   * only today. `available: false` (teams not yet resolved) is not charged —
   * `X-Credits-Used: 0` on the response.
   */
  stats(eventId: number): Promise<StatsResponse> {
    return this.client.get<StatsResponse>(`/v1/events/${eventId}/stats`);
  }
