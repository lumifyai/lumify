...
def test_explicit_key_sets_bearer_and_client_headers():
    ...
    assert cfg["sse_read_timeout"] == DEFAULT_SSE_READ_TIMEOUT


def test_custom_timeouts_in_connection_config():
    ...
    assert cfg["sse_read_timeout"] == 600.0
...
async def test_timeouts_wire_through_get_lumify_tools(monkeypatch):
    ...
    conns = _FakeMCPClient.last_connections
    assert conns["lumify"]["timeout"] == 90.0
    assert conns["lumify"]["sse_read_timeout"] == 600.0
