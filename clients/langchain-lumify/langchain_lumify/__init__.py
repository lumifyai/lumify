...
def _extract_http_status(exc: BaseException) -> int | None:
    ...
    shape (``exc.response.status_code``), walks ``__cause__`` / ``__context__``
    and ``ExceptionGroup.exceptions``, and falls back to pattern-matching the
...
def _tool_call_id_from_request(request: MCPToolCallRequest) -> str:
    """Best-effort ``tool_call_id`` for agent message pairing.

    ``MCPToolCallRequest`` does not carry the id directly; LangGraph may expose
    it on ``request.runtime``. Fall back to empty string when unavailable.
    """
    runtime = request.runtime
    if runtime is None:
        return ""
    for attr in ("tool_call_id", "toolCallId", "id"):
...
def _build_client(
    ...
    *,
    ...
    return MultiServerMCPClient(
        {
            _SERVER_NAME: _connection_config(
                api_key,
                url,
...
async def get_lumify_tools(
    ...
    *,
    ...
    if not tools:
        ...
        # than return an empty list that silently no-ops in an agent.
        raise _empty_tools_error(url)
    return tools
