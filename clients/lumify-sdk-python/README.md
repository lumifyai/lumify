...
Individual connections also close after 5 minutes; the server sends
`event: reconnect` first, and `client.events.stream()` opens a fresh
connection automatically so a long game looks like one continuous stream.
