...
class AsyncEventsResource:
    ...
    async def stats(self, event_id: int) -> Dict[str, Any]:
        ...
        ``credits_used`` is 0 (read via ``get_meta()``). Not yet declared in
        the OpenAPI schema as a typed model, hence the loose ``dict`` shape.
        """
        return await self._client.get("/v1/events/%d/stats" % event_id)
