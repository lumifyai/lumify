...
class AsyncWebhooksResource:
    ...
    async def list(self) -> WebhookListResponse:
        ...
        not re-returned)."""
        return await self._client.get("/v1/webhooks")

    async def delete(self, subscription_id: int) -> WebhookDeleteResponse:
        """DELETE /v1/webhooks/{id}."""
        return await self._client.delete("/v1/webhooks/%d" % subscription_id)
    ...
    def verify(
        ...
        *,
        ...
        """Verify a delivery's ``Lumify-Signature`` header. Pure/local
        computation (stdlib ``hmac``), so this is sync even on the async
        client — no need to ``await`` it. Raises ``WebhookSignatureError`` if
        invalid."""
        verify_webhook(
            signing_secret,
            signature_header,
