...
function main() {
  ...
  const bridge = createBridge({
    url,
    apiKey,
    // Deliberately NOT derived from pkg.name: the published package is the
    // scoped "@lumifyai/mcp", but server-side traffic classification
    // (core/analytics/client_source.py) keys off the stable "lumify-mcp"
    // identifier used everywhere else (bridge.js's own default, docs,
    // package display name). Keep this literal so a future npm rename/rescope
    // can't silently break inbound-client attribution again.
    userAgent: `lumify-mcp/${pkg.version} (node ${process.version})`,
  });
